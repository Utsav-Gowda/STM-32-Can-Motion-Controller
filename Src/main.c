...
/*
 * Two-node CAN control demo for STM32F103 (bare-metal, no HAL).
 * Node A ramps a value and transmits it over CAN; Node B receives it
 * and sets PC13 brightness via software PWM. Node A also reads the
 * MPU-6050 WHO_AM_I register and reports it on every telemetry line.
 *
 * Pick the build role with the ROLE_* define below, flash one board
 * as NODE_A and the other as NODE_B. UART is 9600 8N1 on PA9/PA10.
 */

/*
 * Two-node CAN control demo for STM32F103 (bare-metal, no HAL).
 * Node A ramps a value and transmits it over CAN; Node B receives it
 * and sets PC13 brightness via software PWM. Both nodes stream telemetry
 * over UART (9600 8N1, PA9/PA10).
 *
 * Pick the build role with the ROLE_* define below, then flash one board
 * as NODE_A and the other as NODE_B. ROLE_LOOPBACK runs both halves on a
 * single board using the CAN peripheral's internal loopback (no bus).
 */
